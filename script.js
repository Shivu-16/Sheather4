...
function futureTempData () {
    navigator.geolocation.getCurrentPosition((success) => {
        ...
        fetch(`api.openweathermap.org/data/2.5/forecast/daily?lat=${latitude}&lon=${longitude}&cnt=${8}&unit=metric&appid=${API_KEY}}`).then(Response => Response.json()).then(data => {
...
function showWeatherData (data){
    ...
    let otherDayForcast = ''
    newdata.list.date.forEach((day, idx) => {
        if(idx == 0){
